...
SOCIAL_MEDIA = {"Facebook": "asttravels", "Instagram": "@ast_travels", "Twitter": "@AST_Travels"}
...
class RoundedBox(Flowable):
    def __init__(self, contents, width, height, radius=10, padding=10, 
                 backgroundColor=None, borderColor=None, borderWidth=1):
        Flowable.__init__(self)
    ...
    def draw(self):
        ...
        canvas.roundRect(0, 0, self.width, self.height, self.radius, 
                         stroke=bool(self.borderColor), fill=bool(self.backgroundColor))
...
def header_footer(canvas, doc):
    ...
    try:
        ...
        canvas.drawString(doc.leftMargin, doc.height + doc.topMargin - 0.4*inch, COMPANY_NAME)

    # Header
    canvas.setFont("Helvetica", 10)
    canvas.setFillColor(LIGHT_TEXT_COLOR)
    canvas.drawString(doc.leftMargin + 1.2*inch, doc.height + doc.topMargin - 0.35*inch, COMPANY_SLOGAN)
...
def cover_page(canvas, doc):
    ...
    try:
        cover_img_data = requests.get(IMAGES["cover_image"], stream=True, timeout=10).content
        cover_img_io = BytesIO(cover_img_data)
        canvas.drawImage(cover_img_io, 0, 0, width=A4[0], height=A4[1], preserveAspectRatio=False, mask='auto')
...
def build_pdf():
    ...
    story.append(Paragraph("A Scenic Getaway Through Western Ghats & Coastal Beauty", styles["CoverSubtitle"]))
    
    # Cover page details
    story.append(Spacer(1, 2 * inch))
    price_text = f"Starting from {package_info['Standard']['price']} per person"
    price_para = Paragraph(price_text, ParagraphStyle(
        name="CoverPrice",
    ...
    story.append(Paragraph(duration, ParagraphStyle(
        name="CoverDuration",
        parent=styles["CoverSubtitle"],
        fontSize=16,
    )))
    ...
    udupi_table = Table(
        udupi_content, 
        colWidths=[doc.width - 2.5 * inch - 0.2 * inch, 2.5 * inch]
    )
    ...
    story.append(Paragraph(f"<b>Price:</b> {package_info['Standard']['price']} per person", styles["PackageHighlight"]))
    story.append(Paragraph(f"<b>Duration:</b> {package_info['Standard']['duration']}", styles["BodyText"]))
    story.append(Spacer(1, 0.1 * inch))
    ...
    story.append(Paragraph(f"<b>Price:</b> {package_info['Premium']['price']} per person", styles["PackageHighlight"]))
    story.append(Paragraph(f"<b>Duration:</b> {package_info['Premium']['duration']}", styles["BodyText"]))
    story.append(Spacer(1, 0.1 * inch))
    story.append(Paragraph("<b>Inclusions:</b>", styles["BodyText"]))
    ...
    for i, testimonial in enumerate(testimonials):
        text = Paragraph(f'"{testimonial["text"]}"', styles["TestimonialText"])
        author = Paragraph(f'- {testimonial["author"]}', styles["TestimonialAuthor"])
    ...
    testimonials_table = Table([testimonial_data], 
                              colWidths=[(doc.width - 0.4*inch)/3] * 3,
                              rowHeights=[2.2*inch])
    testimonials_table.setStyle(TableStyle([
