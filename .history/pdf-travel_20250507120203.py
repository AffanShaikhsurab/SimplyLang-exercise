...
def header_footer(canvas, doc):
    ...
    logo_text.drawOn(
        canvas, doc.leftMargin, doc.height + doc.topMargin - h_logo - 0.1 * inch
    )
    ...
    footer_text = Paragraph(
        f"{COMPANY_NAME} | {WEBSITE} | Page {doc.page}", styles["FooterText"]
    )
    w_footer, h_footer = footer_text.wrapOn(canvas, doc.width, doc.bottomMargin)
    footer_text.drawOn(
        canvas, doc.leftMargin, h_footer - 0.1 * inch
    )  # Adjust y position
...
def build_pdf():
    ...
    story.append(
        Paragraph(
            "Your Scenic Getaway Awaits: Karnataka's Treasures", styles["MainTitle"]
        )
    )
    ...
    story.append(
        Paragraph(
            "We look forward to helping you plan your unforgettable journey!",
            styles["BodyText"],
        )
    )
