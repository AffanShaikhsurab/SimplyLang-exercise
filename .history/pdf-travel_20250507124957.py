...
class TimelineFlowable(Flowable):
    ...
    def draw(self):
        ...
        for i, day_info in enumerate(self.days):
            ...
            canvas.roundRect(x - box_width/2, y + 15, box_width, box_height, 3, stroke=0, fill=1)
            ...
            canvas.roundRect(x - act_box_width/2, y + 40, act_box_width, act_box_height, 3, stroke=0, fill=1)
...
class Divider(Flowable):
    def __init__(self, width, color=lightgrey, thickness=1, spacer=0.1 * inch, style="solid"):
        Flowable.__init__(self)
        self.width = width
        self.color = color
...
class DecorationFlowable(Flowable):
    ...
    def draw(self):
        ...
        if self.style == "wave":
            ...
            y_points = [self.height/2 + amplitude * sin(2 * pi * frequency * x / self.width) for x in x_points]
...
class PlaceholderImage(Flowable):
    ...
    def draw(self):
        ...
        self.canv.drawCentredString(self.width/2, self.height/2, "Image Not Available")
...
class StarRating(Flowable):
    ...
    def draw(self):
        ...
        for i in range(self.max_stars):
            if i < int(self.rating):
                ...
                self._draw_star(canvas, i * self.star_size * 1.2, filled=True, fraction=fraction)
            else:
                # Empty star
    ...
    def _draw_star(self, canvas, x_offset, filled=True, fraction=1.0):
        ...
        for i in range(5):
            ...
            angle = (2 * pi * i / 5) - pi / 2
            points.append((center_x + size/2 * cos(angle), center_y + size/2 * sin(angle)))
            
            # Inner points
            angle += pi / 5
            points.append((center_x + size/5 * cos(angle), center_y + size/5 * sin(angle)))
        ...
        if filled:
            ...
            if fraction < 1.0:
                ...
                canvas.setFillColor(self.star_color)
                canvas.rect(center_x - size/2, center_y - size/2, 
                           size * fraction, size, stroke=0, fill=1)
                canvas.restoreState()
...
styles.add(
    ParagraphStyle(
        ...
        spaceAfter=0.15 * inch,  # More space
    )
)
...
package_info = {
    "Standard": {
        ...
        "rating": 4.5,
        "ideal_for": "Couples, Families, Small Groups",
        "includes": [
        ...
        "highlights": "Perfect for budget travelers who want to experience Karnataka's natural beauty without compromising comfort.",
    },
    "Premium": {
        "price": "₹39,999",
        "duration": "7 Days / 6 Nights",
        "rating": 4.8,
        "ideal_for": "Luxury Travelers, Honeymooners",
        "includes": [
        ...
        "highlights": "The ultimate Karnataka experience with personalized service and premium amenities throughout your journey.",
    },
}
...
def build_pdf():
    ...
    two_column_template = PageTemplate(
        id="two_column",
        frames=[
            Frame(doc.leftMargin, doc.bottomMargin, doc.width/2 - 0.25*inch, doc.height, id="col1"),
            Frame(doc.leftMargin + doc.width/2 + 0.25*inch, doc.bottomMargin, doc.width/2 - 0.25*inch, doc.height, id="col2")
        ],
        onPage=header_footer,
    )
    ...
    try:
        image_urls = [IMAGES[key] for key in ["kalasa_bridge", "udupi_beach", "kudremukh_park", "netravati_peak"]]
    ...
    try:
        ...
        map_table = Table(
            [[map_container], 
             [Paragraph("<i>Explore Karnataka's most scenic locations</i>", styles["CaptionText"])]],
            colWidths=[5*inch],
            style=TableStyle([
                ('ALIGN', (0, 0), (-1, -1), 'CENTER'),
                ('VALIGN', (0, 0), (-1, -1), 'MIDDLE'),
    ...
    story.append(Spacer(1, 0.3 * inch))
    story.append(Divider(width=doc.width, style="fancy", color=PRIMARY_COLOR.clone(alpha=0.6)))
    story.append(Spacer(1, 0.3 * inch))
    ...
    # --- Kalasa ---
    story.append(
        Paragraph("Kalasa: Spiritual Charm & Lush Landscapes", styles["SubSectionTitle"])
    )
    ...
    kalasa_table = Table(
        kalasa_content, 
        colWidths=[2.8 * inch, doc.width - 2.8 * inch]
    )
    ...
    story.append(Spacer(1, 0.3 * inch))
    story.append(Divider(width=doc.width, style="dashed", color=PRIMARY_COLOR.clone(alpha=0.6)))
    story.append(Spacer(1, 0.3 * inch))
    ...
    udupi_table = Table(
        udupi_content, colWidths=[doc.width - 2.8 * inch, 2.8 * inch]
    )
    ...
    standard_content.append(
        Paragraph("Standard Package", styles["SubSectionTitle"])
    )
    
    # Price and rating on same line
    price_rating_table = Table(
        [[
            Paragraph(f"<b>Price:</b> {package_info['Standard']['price']} per person", 
    ...
    price_rating_table = Table(
        [[
            Paragraph(f"<b>Price:</b> {package_info['Premium']['price']} per person", 
                    styles["PackageHighlight"]),
    ...
    for i, item in enumerate(package_info["Premium"]["includes"]):
        if i >= len(package_info["Standard"]["includes"]) or item != package_info["Standard"]["includes"][i]:
            # This is a premium-only feature
            premium_inclusions.append(
                ListItem(
                    Paragraph(f"{item} <i>(Premium only)</i>", 
    ...
    contact_info = Table(
        ...
        style=TableStyle([
            ...
            ('BACKGROUND', (0, 0), (0, -1), PALE_BLUE.clone(alpha=0.3)),  # Light background for first column
    ...
    social_table = Table(
        ...
        style=TableStyle([
            ...
            ('BACKGROUND', (0, 0), (0, -1), PALE_YELLOW.clone(alpha=0.3)),  # Light background for first column
            ('TOPPADDING', (0, 0), (-1, -1), 8),
            ('BOTTOMPADDING', (0, 0), (-1, -1), 8),
        ])
    )
