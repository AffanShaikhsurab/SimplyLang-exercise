...
class Parser:
    ...
    def expr(self, seen=False):
        ...
        # Add variable creation logic - handle lexer bug where identifiers are marked as KEYWORD
        if (
            seen == False
            and self.current_token is not None
            and self.current_token.type == lexer.TT_KEYWORD
            and self.current_token.value not in lexer.KEYWORDS
        ):
            temp = self.current_token
            res.register_advance()
            self.advance()
            print("Variable created:", temp.value)

            if not (self.current_token.matches(lexer.TT_KEYWORD, "is") or 
                   (self.current_token.type == lexer.TT_IDENTIFIER and self.current_token.value == "is")):
                return res.failure("Expected 'is'")

            variable: lexer.Token = temp
            res.register_advance()
            self.advance()
        ...
        # Handle simple string/number expressions
        if self.current_token is not None and self.current_token.type == lexer.TT_STRING:
            res.register_advance()
            self.advance()
            return res.success(self.current_token)

        # Handle numbers (stored as keywords due to lexer bug)
        if (self.current_token is not None and 
            self.current_token.type == lexer.TT_KEYWORD and 
            self.current_token.value.isdigit()):
            res.register_advance()
            self.advance()
            return res.success(self.current_token)
