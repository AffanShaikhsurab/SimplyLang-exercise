...
class Parser:
    ...
    def expr(self, seen=False):
        ...
        if not seen and self.current_token.type == lexer.TT_IDENTIFIER:
            ...
            res.register_advance()
            self.advance()
            print("Variable detected:", variable_name)

            # Check for 'is' keyword
            if variable_name.value != "is":
            ...
            if self.current_token.type in (lexer.TT_STRING, lexer.TT_NUMBER):
                ...
                # Add to variables
                var_name = variable_name.value
                if var_name is not None:
                    self.variables.append(var_name)
                return res.success(VariableNode(var_name, expression))
            ...
            if var_name is not None:
                self.variables.append(var_name)
            print("Variable detected:", var_name, "with expression:", expression)
            return res.success(VariableNode(var_name, expression))
