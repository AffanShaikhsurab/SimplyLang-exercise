...
class Parser:
    ...
    def expr(self, seen=False):
        ...
        if (
            ...
            if not (
                self.current_token.matches(lexer.TT_KEYWORD, "is")
                or (
                    self.current_token.type == lexer.TT_IDENTIFIER
        ...
        if (
            ...
            and self.current_token.type == lexer.TT_STRING
        ):
            res.register_advance()
            self.advance()
        ...
        if (
            self.current_token is not None
            and self.current_token.type == lexer.TT_KEYWORD
            and self.current_token.value is not None
            and self.current_token.value.isdigit()
        ):
            res.register_advance()
            self.advance()
            return res.success(self.current_token)
