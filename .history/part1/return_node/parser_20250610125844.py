...
class Parser:
    ...
    def expr(self, seen=False):
        ...
        if (
            ...
            res.register_advance()  # Register the advance first
            self.advance()
            ...
            # Check for simple return with just a value
            if self.index + 1 < len(self.tokens) and self.tokens[self.index + 1].type == lexer.TT_NEWLINE:
                value = self.current_token
                res.register_advance()
                self.advance()
                return res.success(ReturnNode(value))
            else:
                # For complex expressions, we need to avoid recursion
                # Use the term and bin_operation methods instead of expr
                value = res.register(self.term())
                if res.error:
                    return res
                    
                # Handle any binary operations
                if self.current_token and self.current_token.type in (
                    lexer.TT_ADD, lexer.TT_MINUS, lexer.TT_LT, lexer.TT_GT
                ):
                    ...
                    value = BinaryOperationNode(value, op_token, right)
