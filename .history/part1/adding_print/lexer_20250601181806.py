...
class Lex:
    ...
    def iterate(self):
        self.pos.index += 1
        self.pos.col += 1
        self.current = (self.text[self.pos.index - 1] if self.pos.index <= len(self.text) else None)
    ...
    def create_token(self):
        while self.current is not None:
            if self.current in " \t":
                ...
                self.tokens.append(self.make_identifier())            elif self.current in DIGITS:
    ...
    def create_string(self):
        ...
        return Token(TT_STRING, string, pos_start, self.pos)    def make_identifier(self):
        id_str = ""
        pos_start = self.pos.copy()
