...
class Number:
    ...
    def isEquall(self, other):
        if isinstance(other, Number):
            ...
            return None, Lexerer.IllegalOperationError(
...
class Interpreter:
    ...
    def visitBinaryOperationNode(self, node):
        ...
        if left is None or right is None:
            return None
        if node.operator.type == Lexerer.TT_PLUS:
            return left + right
        elif node.operator.type == Lexerer.TT_MINUS:
            return left - right
        elif node.operator.type == Lexerer.TT_MUL:
            return left * right
        elif node.operator.type == Lexerer.TT_DIV:
            return left / right
