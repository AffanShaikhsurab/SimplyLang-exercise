...
class Interpreter:
    ...
    def visit_TillNode(self, node):
        ...
        print("condition_expr: ", node.condition_expr)
        condition = res.register(self.visit(node.condition_expr))
        if res.error:
            return res
        while condition.value is Flase:  # Assuming 0 is false, any other value is true
            ...
            condition = res.register(self.visit(node.condition_expr))
            print(f"Condition after body: {condition}")
        value = None
