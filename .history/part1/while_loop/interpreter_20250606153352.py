...
class Interpreter:
        ...
        while condition.value != 0:  # Assuming 0 is false, any other value is true
            for expr in node.body:
                ...
                if res.error:
                    return res
            condition = res.register(self.visit(node.condition_expr))
        return res.success(None)  # Always return a result object
    ...
    def visit_StatementsNode(
        ...
        for statement in node.statements:
            ...
            # If result is None, skip to the next statement
            if result is None:
                continue
    ...
    def visit_VariableNode(self, node: Pr.VariableNode):
        ...
        value_result = self.visit(node.value_node)
        if value_result is None:
            return res.success(None)
        value = res.register(value_result)
        if res.error:
            return res
        ...
        for statement in node.body:
            ...
            if isinstance(statement, (int, str, bool, float)):
                ...
                if value is None:  # Check for undefined variable
                    print(f"Undefined variable: {statement.variable_name.value}", end=" ")
                    continue
