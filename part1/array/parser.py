...
class Parser:
    ...
    def expr(self, seen=False):
        ...
        if not seen and self.current_token.type == lexer.TT_IDENTIFIER:
            ...
            # Array assignment with "at" keyword (e.g., arr at 0 is value)
            if (
                self.current_token.type == lexer.TT_KEYWORD
                and self.current_token.value == "at"
            ):
                self.advance()
                ...
                if res.error:
                    return res
                return res.success(ArrayVariable(variable_token, index, expression))

            # Array assignment with bracket syntax (e.g., arr[0] is value)
            ...
            # Handle array creation and other special keywords
            if (
                self.current_token.type == lexer.TT_KEYWORD
                and self.current_token.value == "arrange"
            ):
                self.advance()
                ...
                if res.error:
                    return res
                self.variables.append(variable_token.value)
                ...
                if (
                    ...
                    if (
                        self.current_token.type == lexer.TT_KEYWORD
                        and self.current_token.value == "descending"
                    ):
                ...
                return res.success(
                    ArrayArrangeNode(
                        variable_token, expression.variable_name, arrange_ascending
                    )
                )

            if (
                self.current_token.type == lexer.TT_KEYWORD
                and self.current_token.value == "length"
            ):
                self.advance()
                if (
                    self.current_token.type != lexer.TT_KEYWORD
                    or self.current_token.value != "of"
                ):
                    return res.failure(
                        lexer.InvalidSyntaxError(
                            "Expected 'of'",
                ...
                self.variables.append(variable_token.value)
                return res.success(ArrayLengthNode(variable_token, expression.variable_name))
            ...
            if self.current_token.type in (
                ...
                # Complex expression - handle array creation and array access
                expression = res.register(self.expr(True))
                if res.error:
                    return res

                # Check for array access with "at" keyword
                if (
                    self.current_token.type == lexer.TT_KEYWORD
                    and self.current_token.value == "at"
                ):
                    ...
                    self.variables.append(variable_token.value)
                    return res.success(
                        ArrayAccessNode(variable_token, index, expression.variable_name.value)
                    )

                # Check for array access with bracket syntax
                if self.current_token.type == lexer.TT_LBRACKET:
                    self.advance()
                    if self.current_token.type != lexer.TT_INT:
                        return res.failure(
                ...
                # Check for array creation with comma-separated values
                if self.current_token.type == lexer.TT_COMMA:
                    array = []
                    array.append(expression.token.value)
                    
                    while (
                        self.current_token.type != lexer.TT_EOF
                    ...
                    self.variables.append(variable_token.value)
                    return res.success(ArrayNode(variable_token, array))
